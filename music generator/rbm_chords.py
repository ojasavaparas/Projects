...
W_adder  = tf.scalar_mul(lr/size_bt, tf.subtract(tf.matmul(tf.transpose(x), h), tf.matmul(tf.transpose(x_sample), h_sample)))
bv_adder = tf.scalar_mul(lr/size_bt, tf.reduce_sum(tf.subtract(x, x_sample), 0, True))
bh_adder = tf.scalar_mul(lr/size_bt, tf.reduce_sum(tf.subtract(h, h_sample), 0, True))
